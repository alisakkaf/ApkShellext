                ...
                for (int i = 0; i < token.Count();i++) {
                    ...
                    switch (command) {
                        ...
                        case 'a':
                            ...
                            double cosf = Math.Cos(f), sinf = Math.Sin(f);
                            double[,] x1_y1 = MultiplyMatrix(new double[,] { { cosf, sinf },
                                                                             { -sinf, cosf } },
                            ...
                            double cx = cosf * cx_ - sinf * cy_ + (startpoint.X + endpoint.X) / 2;
                            double cy = sinf * cx_ + cosf * cy_ + (startpoint.Y + endpoint.Y) / 2;
                            PointF center = new PointF((float)cx, (float)cy);

                            //Step 4: Compute ?1 and ??
                            //double ?1 = Math.Acos((y1_ - cy_) * ry / (length(new double[] { 1, 0 }) * length(new double[] { (x1_ - cx_) / rx, (y1_ - cy_) / ry })));
                            double ?1 = VectorAngle(new double[,] { { 1 }, { 0 } },
                                new double[,] {
                                { (x1_ - cx_ / rx) },
                                { (y1_ - cy / ry) }
                                });
                            ...
                            path.AddArc(new Rectangle((int)(cx - rx), (int)(cy - ry), 2 * (int)rx, 2 * (int)ry), (float)?1, (float)delta?);
                            trans.RotateAt((float)-xrotate, center);
                            path.Transform(trans);
                            break;
                        case 'Z':
